function initMountain() {
    ...
    // PERF_MODE: reduce effects on touch + narrow viewports
    const PERF_MODE = (() => {
        const isTouch = 'ontouchstart' in window || navigator.maxTouchPoints > 0;
        const isSmall = window.innerWidth < 860;
        const prefersReduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
        return (isTouch && isSmall) || prefersReduced;
    })();
    ...
    const PEAK_CAMPS = [{
        ...
        skills: [
            ...
            { n: 'Godot', lvl: 'comfortable' }, { n: 'Unity Editor', lvl: 'comfortable' }, { n: 'Git', lvl: 'comfortable' },
            { n: 'GitHub', lvl: 'comfortable' },
    ...
    const celebrationCanvas = document.getElementById('summitCelebration');
    ...
    // ── Build content ──
    function buildCampsHtml() {
        return PEAK_CAMPS.map((camp, ci) =>
    ...
    function buildClimbingHtml() {
        return PEAK_CLIMBING.map(c =>
    ...
    // Defer heavy DOM building to the next frame so the panel transition starts cleanly.
    requestAnimationFrame(() => {
        ...
        const CLOUD_COUNT = PERF_MODE ? 5 : 10;
        for (let i = 0; i < CLOUD_COUNT; i++) {
            const c = createCloud();
            c.x = -c.width - Math.random() * 300 + (i / CLOUD_COUNT) * window.innerWidth * 0.8;
        ...
        function updateActiveCamp() {
            ...
            if (idx === -1 || idx === activeCampIdx) return;
            activeCampIdx = idx;
        ...
        function updateProgress() {
            ...
            if (Math.abs(scrollTop - lastScrollTop) < 2 && Math.abs(pct - lastPct) < 0.005) return;
            ...
            gradientEl.style.background =
                `linear-gradient(180deg, rgb(${r1},${g1},${b1}) 0%, rgb(${r2},${g2},${b2}) 30%, rgb(${r3},${g3},${b3}) 60%, rgb(${r4},${g4},${b4}) 100%)`;
            ...
            // parallax layers
            if (layerMountains) {
                layerMountains.style.transform = `translateY(${scrollTop * 0.08}px) scale(1.02)`;
            }
            if (layerRidges) {
                layerRidges.style.transform = `translateY(${scrollTop * 0.04}px) scale(1.01)`;
            }
            if (layerMist) {
                layerMist.style.transform = `translateY(${scrollTop * 0.02}px)`;
            }
            if (layerGlow) {
                const intensity = 0.5 + pct * 0.4;
                layerGlow.style.opacity = intensity;
                layerGlow.style.transform = `translateY(${scrollTop * 0.01}px)`;
            }

            updateActiveCamp();

            // summit celebration
            if (!window._summitCelebrated && !window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
                const rect = summitEl.getBoundingClientRect();
                if (rect.top < window.innerHeight * 0.4 && rect.bottom > window.innerHeight * 0.6) {
                    window._summitCelebrated = true;
                    triggerSummitCelebration();
                }
            }
        }

        // ── scroll handler with rAF coalescing ──
        ...
        function animateCelebration() {
            ...
            celebrationParticles.forEach(p => {
                ...
                if (!PERF_MODE) { ctx.shadowColor = p.color;
                    ctx.shadowBlur = 16; }
                ctx.beginPath();
